...
function macofelAgent(data) {
  if (!data?.ok) {
    return agent('macofel', 'Macofel', 'Catálogo e-commerce', 'error', data?.error || 'indisponível');
  }
  ...
  if (pending > 0 || imgPending > 0) {
    ...
    return agent('macofel', 'Macofel', 'Catálogo e-commerce', 'working', parts.join(' · '), {
      pending_review: pending,
      image_sync_pending: imgPending,
      source: data.source,
    });
  }
  return agent('macofel', 'Macofel', 'Catálogo e-commerce', 'idle', 'catálogo em dia', {
    source: data.source,
    total: data.total,
  });
}
...
function vpPecasAgent(deploy) {
  ...
  if (!site) {
    return agent('vp-pecas', 'VP-Pecas', 'Sites usinagem', 'thinking', 'URL não configurada');
  }
  if (!site.ok) {
    return agent('vp-pecas', 'VP-Pecas', 'Sites usinagem', 'error', site.error || `HTTP ${site.status}`, {
      url: site.url,
    });
  }
  if (site.ms && site.ms > 4000) {
    return agent('vp-pecas', 'VP-Pecas', 'Sites usinagem', 'thinking', `lento (${site.ms}ms)`, {
      url: site.url,
      ms: site.ms,
    });
  }
  return agent('vp-pecas', 'VP-Pecas', 'Sites usinagem', 'idle', `online (${site.ms ?? '?'}ms)`, {
    url: site.url,
    status: site.status,
  });
}
...
function orchestratorAgent(macofel, github, deploy) {
  const subs = [macofelAgent(macofel), vpPecasAgent(deploy), opsAgent(github, deploy)];
  if (subs.some((a) => a.state === 'error')) {
    return agent('orchestrator', 'Jarvis', 'Orquestrador', 'error', 'incidente no portfólio');
  }
