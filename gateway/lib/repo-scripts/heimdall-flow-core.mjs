...
export async function fetchHubAgentActivity({ limit = 40 } = {}) {
  try {
    const { isHubEnabled, fetchRecentHub } = await import('../hub-store.mjs');
    if (!isHubEnabled()) {
...
async function fetchOfficeSnapshotSafe() {
  ...
  try {
    const { fetchOfficeSnapshot } = await import('../office.mjs');
...
function snapshotPath() {
  return resolve(openclawDataRoot(), 'heimdall', 'last-flow.json');
}
