/**
 * Lembretes WhatsApp (Twilio) — só para TWILIO_WHATSAPP_TO (admin).
 */
import {
  readFileSync,
  writeFileSync,
...
export function getTwilioConfig() {
  ...
  if (!accountSid || !authToken || !from || !to) {
    return {
      ok: false,
      error:
        'Twilio incompleto: TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_WHATSAPP_FROM, TWILIO_WHATSAPP_TO no .env',
    };
  }
  return {
    ok: true,
 ...
 */
export async function sendTwilioWhatsApp(payload, config = getTwilioConfig()) {
...
export function scheduleMessage({ body, sendAt, mediaUrl = '' }, file = DEFAULT_QUEUE_FILE) {
...
export async function dispatchDue({ dryRun = false, file = DEFAULT_QUEUE_FILE } = {}) {
  ...
  for (const item of queue.items) {
    ...
    const result = await sendTwilioWhatsApp(outbound, cfg);
    if (result.ok) {
      item.status = 'sent';
...
export async function handleScheduleCommand(
  ...
  if (/^(sim|confirmar|ok)\b/i.test(text) && text.length < 48) {
    ...
    if (pending) {
      ...
      const r = scheduleMessage(
        { body: pending.body, sendAt, mediaUrl: pending.mediaUrl },
        file
      );
  ...
  if (parsed.action === 'list') {
    ...
    const lines = items.map(
      (i) => `• ${i.id} — ${formatSp(i.sendAt)} — ${i.body.slice(0, 80)}`
    );
